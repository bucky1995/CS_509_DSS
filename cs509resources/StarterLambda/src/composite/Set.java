...
public class Set extends Element implements Iterable<Element> {
	...
	public boolean doesContain (Element e) { 
		
		// given e I need to check equality with all my direct children.
		
		for (Iterator<Element> it = iterator(); it.hasNext(); ) {
			Element elt = it.next();
			if (elt.equals(e)) { return true; } // I HAVE IT DIRECTLY
			
			if (elt.doesContain(e)) {   // I MIGHT HAVE IT RECURSIVELY
				return true;
			}
		}
